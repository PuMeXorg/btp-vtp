...
return new class extends Migration
{
    ...
    {
        // Get or create parent "Насосные станции"
        ...
        // Move "Насосные станции пожаротушения" under parent
        DB::table('pages')
            ->where('slug', 'nasosnye-stantsii-pozharotusheniya')
            ->update(['parent_id' => $parentId, 'sort' => 2, 'updated_at' => now()]);

        // Upsert "Насосные станции поддержания давления"
        $pdd = DB::table('pages')->where('slug', 'nasosnye-stantsii-podderzhania-davlenia')->first();
        ...
        if ($pdd) {
            DB::table('pages')->where('id', $pdd->id)->update([
                'parent_id'  => $parentId,
                'sort'       => 3,
                'content'    => $contentPDD,
                'is_active'  => 1,
        ...
        // Upsert "Повысительные насосные станции"
        $pov = DB::table('pages')->where('slug', 'povysitelnye-nasosnye-stantsii')->first();
        ...
        if ($pov) {
            DB::table('pages')->where('id', $pov->id)->update([
                'parent_id'  => $parentId,
                'sort'       => 1,
                'content'    => $contentPov,
                'is_active'  => 1,
